...
> **架构定位**：Harness 引擎的独立自护子系统，专门管控 Harness 自身的基础设施变更。
>
...
在 Harness 引擎环境中发送以下**任意一条**消息即可激活 SelfGuard 自护模式：
...
**激活后行为**：所有 `src/`、`data/`、`self_guard/` 目录下的文件变更自动路由至 SelfGuard 自护流水线。
