...
- 日期：2026-09-09/10；状态：**🔴+部分🟠 已落地 live；H-01/H-02 收口与 M 接入转专项（见 03 设计文档）**
...
| 债务散落无台账 | 🟠 **H-03 半落地** | 台账 DB + CRUD + S4.5 DS<98→候选池(live)；**S2 补丁→自动建 debt 未接**（依赖 M-03 s2-evidence-v2 接入，转专项） |
| S3 整文件覆写无硬闸 | 🟠 **H-05 核心 live** | writeRatio 模块 + harness_globals.json 阈值 + ToolWhitelistGuard checkWriteWithRatio/allowlist/audit（进程内调用点接线待专项） |
| S7 归档无机器校验 | 🟠 **H-01 校验器就绪，未接线** | S7ArchiveValidator(R1-R5,6测试)；接线设计见 03 |
| S6 行为验证降级"参考" | ⬜ **H-02 未做** | run_status 字段已扩展；S6-A/B + await 语义需引擎暂停态（见 03） |
| CK-06.5/DS-23 名称碰撞 | ✅（会话早前） | B：systemic 命中→warn+候选 |
| 评审义务/确认死锁 | ✅（会话早前） | P0-A + P0-A2 |
| EvolutionEngine 空转 | ✅（会话早前） | P1：audit 接通（0→4701 事件） |

## 二、M 骨架（🟡 仅骨架，未接入）
- ContractResolver.ts（接口/抛错桩）、core_constraint_mapping.yaml、overlays/wenstar_v1.0_overlay.yaml、4 组 schema（TS+json-schema）、tech_debt_ledger 模块、S7ArchiveValidator
- 全部带 `🟡MEDIUM_SKELETON_ONLY` 注释；L 系列仅注释

## 三、git 变更清单（本次 enhance-v1 涉及）

**新增文件**：`src/debt/techDebtLedger.ts`｜`src/schemas/{s2-evidence-v2,full-review-evidence,s7-archive-payload,manual-verification-ticket}.ts + .schema.json`｜`src/writeguard/writeRatio.ts`｜`src/contract/ContractResolver.ts`｜`src/s7/S7ArchiveValidator.ts`｜`scripts/harness-debt-schema.cjs`｜`scripts/harness-debt-migrate.cjs`｜`data/harness_globals.json`｜`harness_contract/{core_constraint_mapping.yaml, overlays/wenstar_v1.0_overlay.yaml}`｜`docs/harness/{00,01,task-harness-enhance-v1,03_S6S7拆分实施设计-待执行,本报告}`｜测试 4 个
...
| 全量干净回归 | ✅ | `tsc --noEmit` 0 错误；`vitest run` **597/597 通过**（33 文件，约 29s） |
